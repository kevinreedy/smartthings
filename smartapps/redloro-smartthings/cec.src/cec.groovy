/**
 *  SmartThings SmartApp: CEC SmartApp to be used with CEC Device Type and SmartThings Node Proxy CEC Plugin
 *
...
def lanResponseHandler(evt) {
  ...
  if (headers.'stnp-plugin' != 'cec') {
    return
  }

  //log.trace "CEC event: ${evt.stringValue}"
  processEvent(body)
}

private sendCommand(path) {
  //log.trace "CEC send command: ${path}"
...
private addChildDevices() {
  ...
  def deviceId = 'cec|device1'
  if (!getChildDevice(deviceId)) {
    addChildDevice("redloro-smartthings", "CEC Device", deviceId, hostHub.id, ["name": "CEC Device", label: "CEC Device", completedSetup: true])
...
private updateDevices(evt) {
  ...
  def device = getChildDevice("cec|device${evt.deviceId}")
  if (device) {
    device.update(evt)
  }
}
